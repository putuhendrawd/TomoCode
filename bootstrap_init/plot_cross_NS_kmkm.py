...
proj = pygmt.project(center=f'{lon_min+(lon_max-lon_min)/2}/{lat_min}',
                     endpoint=f'{lon_min+(lon_max-lon_min)/2}/{lat_max}',
                     unit=True,
                     generate='0.1')
lat_length_max = round(proj.p.max())
...
for mm in id:
    try:
        ...
        ell = Ellipse(xy=(np.mean(y_proj), np.mean(z)),
        ...
        ell.set_linewidth(1)
        ax.add_artist(ell)
        # ax.scatter(np.mean(y), np.mean(z), 0.1, 'r')
        ...
        # lambdaz1[np.isnan(lambdaz1)] = 0
        # theta=np.rad2deg(np.arccos(v[0, 0]))
        #print(np.mean(y),np.mean(z),tempx,tempz,theta)
    except:
        pass
    ll += 1
...
fig.savefig(path+'output/MapView_bootstrap1_NS_kmkm.png', dpi=500) 
# fig.savefig('output/MapView_bootstrap1_NS.eps', dpi=500)
