...
if os.path.isfile(path+Path(arrival_fname).stem+".csv"):
...
for i, line in enumerate(lines):
    ...
    # phase arrival magnitude counter 
    if trigger_d['phase']:
        if len(line.split()) > 3 and "sta" not in line.lower():
            # print(line)
            net_ = line.split()[1]
            sta_ = line.split()[0]
